...
class Detection:
    ...
    def load_ballModel(self):
        #  Load ballModel
        ballModel = YOLO("redBall_n_openvino_model", task='detect')
        # ballModel.fuse()
        return ballModel
    

    def load_siloModel(self):
        ncnn_model = YOLO("silo_3_class_openvino_model", task='detect')
        # ncnn_model.fuse()
        return ncnn_model


    def predict(self, frame, frame2):
        # Implementing ballModel to the frame
        ballResults = self.ballModel(frame, conf = 0.25)
        siloResults = self.siloModel(frame2, conf = 0.25)
    ...
    def plot_ball_bboxes(self, results, frame):
        ...
        # Setting the center of frame for error parameter
        ...
        for result in results:
            ...
            for x, y, x1, y1 in xyxys:
                ...
                # Calculate the objects bounding box area
                areaBoxes = (y1-y)*(x1-x)

                # condition to only draw a bbox to valid object
                # if nonZero >= (areaBoxes/6):

                # Extracting the error of the ball
                ...
                # Filtering the biggest/closest object to the robot
        ...
        # Return the coordinate and error of the object
    ...
    def __call__(self):
        ...
        while True:
            ...
            # Using the Model
            predBallFrame, predSiloFrame = self.predict(frame, frame2)
            ...
            if(a % 2 == 0):
                serialInst.write('M'.encode())
                serialInst.write(f'{errBall}\n'.encode())
            else:
                serialInst.write('N'.encode())
                serialInst.write(f'{errSilo}\n'.encode())     
            try:
...
portVar = "COM4"
serialInst = serial.Serial(port=portVar, baudrate=115200, timeout=0)

# 0 capture index for ball camera, 2 capture index for silo camera
